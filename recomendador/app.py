...
# RUTAS PARA COLAB / DRIVE
PATH_DRIVE = "/Clubes-de-lectura-navarra/recomendador"
URL_LOGO = f"{PATH_DRIVE}/logo_B. Navarra.jpg"
URL_BOTON_RANDOM = f"{PATH_DRIVE}/serendipia.png"
PATH_FEEDBACK = f"{PATH_DRIVE}/feedback_tesis.csv"
...
@st.cache_resource
def load_resources():
    df_ia = pickle.load(open(f"{PATH_DRIVE}/metadatos_promptss_infloat_ponderado_genero.pkl", "rb"))
    df_ia['Nº lote'] = df_ia['Nº lote'].astype(str).str.strip()
    
    excel_path = f"{PATH_DRIVE}/CATALOGO_VALIDADO_FINAL1.xlsx"
    ...
    index = faiss.read_index(f"{PATH_DRIVE}/biblioteca_prompts_infloat_ponderado_genero.index")
...
def mostrar_card(r, context):
    with st.container(border=True):
        ...
        with col_img:
            ...
            archivos = glob.glob(f"{PATH_DRIVE}/portadas_biblioteca/{lote_id}.*")
