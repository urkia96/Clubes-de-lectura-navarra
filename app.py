...
with st.sidebar.expander(t["exp_gral"], expanded=False):
    # Cargamos opciones únicas de la columna de idioma (Castellano o Euskera según corresponda)
    f_idioma = st.multiselect(t["f_idioma"], sorted([i for i in df[col_idioma].unique() if i != "Desconocido"]))
    
    # Público (Infantil, Heldua, etc.)
    f_publico = st.multiselect(t["f_publico"], sorted([p for p in df[col_publico].unique() if p != "Desconocido"]))
    
    # Género del autor/a
    f_gen_aut = st.multiselect(t["f_genero_aut"], sorted([g for g in df[col_gen_aut].unique() if g != "Desconocido"]))
    
    # Editorial (esta se mantiene igual para ambos idiomas)
...
# 5.2 FILTROS DE CONTENIDO (Basados en IA y traducidos)
with st.sidebar.expander(t["exp_cont"], expanded=False):
    ...
    if f_ia_gen:
        subs = set()
        # Buscamos subgéneros dentro de los géneros seleccionados (usando columnas traducidas)
...
def filtrar(dataframe):
    temp = dataframe.copy()
    # Aplicamos filtros usando las variables dinámicas de columna (col_...)
    ...
    if f_ia_sub: 
        # Lógica para buscar subgéneros dentro de strings separados por comas
        temp = temp[temp[col_ia_sub].apply(lambda x: any(s in str(x) for s in f_ia_sub) if pd.notnull(x) else False)]
    return temp
